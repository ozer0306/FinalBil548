...
#import base64
from .KDCServer import KDCServer
from .ClientEncrytionJS import ClintJS
...
@main.route('/chatRoom',methods = ['POST', 'GET'])
def result():
   if request.method == 'POST':

      print(request.form['submit_button'])

      roomId = KDCServer.getRoomIdForClient(request.form['submit_button'])

      sessionId  = uuid.uuid1().bytes
      clientEncyrpt = ClintJS.encrytRoomId(roomId,sessionId)
      serverEncyrypt = KDCServer.getRoomId(request.form['submit_button'],sessionId)
      if clientEncyrpt==serverEncyrypt:
          return render_template("insideChatRoom.html")
